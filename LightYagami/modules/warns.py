...
import telegram
from LightYagami import TIGERS, WOLVES, dispatcher
from LightYagami.modules.disable import DisableAbleCommandHandler
from LightYagami.modules.helper_funcs.chat_status import (bot_admin,
                                                           can_restrict,
                                                           is_user_admin,
                                                           user_admin,
                                                           user_admin_no_reply)
from LightYagami.modules.helper_funcs.extraction import (extract_text,
                                                          extract_user,
                                                          extract_user_and_text)
from LightYagami.modules.helper_funcs.filters import CustomFilters
from LightYagami.modules.helper_funcs.misc import split_message
from LightYagami.modules.helper_funcs.string_handling import split_quotes
from LightYagami.modules.log_channel import loggable
from LightYagami.modules.sql import warns_sql as sql
...
def warn(user: User,
    ...
    if user.id in TIGERS:
        if warner:
            ...
            message.reply_text(
                "SUDO triggered an auto warn filter!\n I can't warn SUDO but they should avoid abusing this."
            )
        return

    if user.id in WOLVES:
        if warner:
            message.reply_text("SUDO are warn immune.")
        else:
            message.reply_text(
                "SUDO triggered an auto warn filter!\nI can't warn SUDO but they should avoid abusing this."
            )
